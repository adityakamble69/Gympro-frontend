...
function StatusBadge({ p }) {
  ...
  if (isPartial) return <span style={{ padding:"2px 8px", borderRadius:"99px", fontSize: "11px", fontWeight:700, background:"var(--yellow-bg)", color:"var(--yellow)" }}>Partial ⚠️</span>;
  if (p.status === "paid") return <span style={{ padding:"2px 8px", borderRadius:"99px", fontSize: "11px", fontWeight:700, background:"var(--green-bg)", color:"var(--green)" }}>Paid ✅</span>;
  return <span style={{ padding:"2px 8px", borderRadius:"99px", fontSize: "11px", fontWeight:700, background:"var(--red-bg)", color:"var(--red)" }}>Pending ❌</span>;
}
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", borderBottom: "1px solid var(--border-subtle)", overflowX: "auto" }}>
      {[
        ...
        <div key={c.label} style={{ padding: "10px 12px", textAlign: "center", borderRight: "1px solid var(--border-subtle)", background: "var(--bg-elevated)", minWidth: "80px" }}>
          ...
          <div style={{ fontFamily: "var(--font-display)", fontSize: "19px", fontWeight: 800, color: c.color }}>{c.value}</div>
          <div style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "2px", textTransform: "uppercase", letterSpacing: "0.06em" }}>{c.label}</div>
        </div>
      ))}
    </div>
  );
...
function PaymentsTable({ payments, search, filter }) {
  ...
  if (filtered.length === 0)
    return <div style={{ padding: "48px", textAlign: "center", color: "var(--text-muted)", fontSize: "15px" }}>No payments found</div>;
  ...
  return (
    <>
          ...
          {filtered.map((p, idx) => (
            ...
            >
              ...
              <td style={{ padding: "11px 14px" }}>
                ...
                <div style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "1px" }}>{p.phone}</div>
              </td>
              ...
              <td style={{ padding: "11px 14px", color: "var(--text-muted)", whiteSpace: "nowrap", fontSize: "12px" }}>{fmtDate(p.payment_date)}</td>
      ...
      <div style={{ padding: "12px 20px", borderTop: "1px solid var(--border-subtle)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
        <span style={{ fontSize: "12px", color: "var(--text-muted)" }}>Showing {filtered.length} of {payments.length} records</span>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <span style={{ fontSize: "13px", color: "var(--text-muted)" }}>Grand Total:</span>
          <span style={{ fontFamily: "var(--font-display)", fontSize: "18px", fontWeight: 800, color: "var(--green)" }}>{fmt(grandTotal)}</span>
        </div>
      </div>
    </>
  );
  ...
  return (
    <>
      <div style={{ padding: "14px 24px", borderBottom: "1px solid var(--border-subtle)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "var(--bg-elevated)" }}>
        <div>
          <div style={{ fontFamily: "var(--font-display)", fontSize: "17px", fontWeight: 800, color: "var(--text-primary)" }}>{title} — {year}</div>
          <div style={{ fontSize: "12px", color: "var(--text-muted)", marginTop: "2px" }}>Total: {fmt(totalRevenue)}</div>
        </div>
      </div>
            ...
            <thead>
              <tr style={{ background: "var(--bg-elevated)" }}>
                {["Month", "Revenue", "Transactions", "Collected", "Due"].map(h => (
                  <th key={h} style={{ padding: "10px 16px", textAlign: "left", fontSize: "12px", fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.08em", borderBottom: "1px solid var(--border-subtle)" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {months.map(m => (
                ...
                >
                  <td style={{ padding: "13px 16px", fontWeight: 700, color: "var(--text-primary)" }}>
                    {MONTH_NAMES[m.month]} <span style={{ fontSize: "11px", color: "var(--text-muted)", marginLeft: "6px" }}>→ view details</span>
                  </td>
                  <td style={{ padding: "13px 16px", fontFamily: "var(--font-display)", fontWeight: 800, color: "var(--green)", fontSize: "17px" }}>{fmt(m.total)}</td>
          ...
          <tbody>
            {years.map(y => (
              <tr key={y.year} onClick={() => onYearClick(y.year)}
                ...
                onMouseLeave={e => e.currentTarget.style.background = "transparent"}
              >
                <td style={{ padding: "13px 16px", fontFamily: "var(--font-display)", fontWeight: 800, color: "var(--text-primary)", fontSize: "18px" }}>
                  {y.year} <span style={{ fontSize: "11px", color: "var(--text-muted)", marginLeft: "6px" }}>→ view months</span>
                </td>
                <td style={{ padding: "13px 16px", fontFamily: "var(--font-display)", fontWeight: 800, color: "var(--green)", fontSize: "17px" }}>{fmt(y.total)}</td>
...
function DateRangeView({ from, to, onBack, cachedGet }) {
  ...
  const inp = { padding: "8px 12px 8px 30px", background: "var(--bg-elevated)", border: "1px solid var(--border-default)", borderRadius: "var(--radius-sm)", color: "var(--text-primary)", fontSize: "13px", outline: "none", width: "100%", boxSizing: "border-box" };
  ...
  return (
    <>
      ...
      <div style={{ padding: "12px 20px", borderBottom: "1px solid var(--border-subtle)", display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: "6px" }}>
          {["all", "paid", "partial", "pending"].map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ padding: "6px 14px", borderRadius: "99px", fontSize: "12px", fontWeight: 600, cursor: "pointer", textTransform: "capitalize", transition: "all 0.15s", background: filter === f ? "var(--purple)" : "var(--bg-elevated)", color: filter === f ? "#fff" : "var(--text-muted)", border: filter === f ? "none" : "1px solid var(--border-default)" }}>{f}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {loading ? (
          <div style={{ padding: "48px", textAlign: "center", color: "var(--text-muted)", fontSize: "15px" }}>Loading date range data...</div>
        ) : (
      ...
      <div onClick={e => e.stopPropagation()} className="rdd-modal" style={{ background: "var(--bg-surface)", borderRadius: "var(--radius-lg)", border: "1px solid var(--border-default)", width: "min(960px,96vw)", maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 32px 80px rgba(0,0,0,0.7)", animation: "reportModalIn 0.2s cubic-bezier(0.16,1,0.3,1)" }}>
        ...
        <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border-subtle)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "var(--bg-elevated)", borderRadius: "var(--radius-lg) var(--radius-lg) 0 0", flexShrink: 0, gap: "8px", flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", minWidth: 0, flex: 1 }}>
            <button onClick={goBack} style={{ display: "flex", alignItems: "center", gap: "5px", padding: "5px 8px", borderRadius: "var(--radius-sm)", background: "var(--bg-surface)", border: "1px solid var(--border-default)", color: "var(--text-muted)", cursor: "pointer", fontSize: "12px", flexShrink: 0 }}>
              <FaChevronLeft style={{ fontSize: "11px" }} /> {stack.length <= 1 ? "Close" : "Back"}
            </button>
            {/* Breadcrumb */}
            <div style={{ display: "flex", alignItems: "center", gap: "4px", fontSize: "13px", minWidth: 0, overflow: "hidden" }}>
              {stack.map((s, i) => (
            ...
            <div style={{ position: "relative" }}>
              ...
              {showDatePicker && (
                <div onClick={e => e.stopPropagation()} style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: "var(--bg-elevated)", border: "1px solid var(--border-strong)", borderRadius: "var(--radius-md)", padding: "16px", zIndex: 100, width: "min(280px, 90vw)", boxShadow: "0 12px 32px rgba(0,0,0,0.5)" }}>
                  <div style={{ fontSize: "13px", fontWeight: 700, color: "var(--text-secondary)", marginBottom: "12px", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                    📅 Select Date Range
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "14px" }}>
                    <div>
                      <label style={{ display: "block", fontSize: "11px", color: "var(--text-muted)", marginBottom: "4px", textTransform: "uppercase", letterSpacing: "0.08em" }}>From</label>
                      <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
                        style={{ width: "100%", padding: "8px 12px", boxSizing: "border-box", background: "var(--bg-surface)", border: "1px solid var(--border-default)", borderRadius: "var(--radius-sm)", color: "var(--text-primary)", fontSize: "15px", outline: "none" }}
                        onFocus={e => e.target.style.borderColor = "#a78bfa"}
                        onBlur={e => e.target.style.borderColor = "var(--border-default)"}
                      />
                    </div>
                    <div>
                      <label style={{ display: "block", fontSize: "11px", color: "var(--text-muted)", marginBottom: "4px", textTransform: "uppercase", letterSpacing: "0.08em" }}>To</label>
                  ...
                  <div style={{ marginBottom: "12px" }}>
                    <div style={{ fontSize: "11px", color: "var(--text-muted)", marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.06em" }}>Quick Select</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
                      {[
                        ...
                        <button key={p.label} onClick={() => { setDateFrom(p.from); setDateTo(p.to); }}
                          style={{ padding: "4px 10px", borderRadius: "99px", fontSize: "12px", fontWeight: 500, cursor: "pointer", background: dateFrom === p.from && dateTo === p.to ? "rgba(167,139,250,0.2)" : "var(--bg-surface)", border: `1px solid ${dateFrom === p.from && dateTo === p.to ? "#a78bfa" : "var(--border-default)"}`, color: dateFrom === p.from && dateTo === p.to ? "#a78bfa" : "var(--text-muted)", transition: "all 0.12s" }}>
                          {p.label}
                        </button>
                      ))}
                    </div>
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    <button onClick={() => setShowDatePicker(false)} style={{ flex: 1, padding: "8px", borderRadius: "var(--radius-sm)", background: "var(--bg-surface)", border: "1px solid var(--border-default)", color: "var(--text-muted)", cursor: "pointer", fontSize: "13px" }}>Cancel</button>
                    <button onClick={applyDateRange} disabled={!dateFrom || !dateTo}
                      style={{ flex: 1, padding: "8px", borderRadius: "var(--radius-sm)", background: dateFrom && dateTo ? "#a78bfa" : "var(--bg-elevated)", color: dateFrom && dateTo ? "#fff" : "var(--text-muted)", border: "none", cursor: dateFrom && dateTo ? "pointer" : "not-allowed", fontSize: "13px", fontWeight: 700 }}>
                      Apply Filter
                    </button>
                  </div>
                </div>
        ...
        {showSearchFilter && (
          <div style={{ padding: "12px 20px", borderBottom: "1px solid var(--border-subtle)", display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap", flexShrink: 0 }}>
            ...
            <div className="rdd-filter-pills" style={{ display: "flex", gap: "6px" }}>
              {["all", "paid", "partial", "pending"].map(f => (
                <button key={f} onClick={() => setFilter(f)} style={{ padding: "6px 14px", borderRadius: "99px", fontSize: "12px", fontWeight: 600, cursor: "pointer", textTransform: "capitalize", transition: "all 0.15s", background: filter === f ? "var(--text-primary)" : "var(--bg-elevated)", color: filter === f ? "#0a0a0a" : "var(--text-muted)", border: filter === f ? "none" : "1px solid var(--border-default)" }}>{f}</button>
              ))}
            </div>
          </div>
        )}
        ...
        <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column" }}>
          ...
          {!isDateRangeView && (
            loading ? (
              <div style={{ padding: "48px", textAlign: "center", color: "var(--text-muted)", fontSize: "15px" }}>Loading...</div>
            ) : (
              <>
